...
- AI chat bookkeeping is dual-keyed by **host** pane id: `pane_state.overlay` and `ai_chat_overlay_panes`. Cancel/clear/`PaneRemoved` must go through `cancel_overlay_for_pane(host_id)` — never the allocated overlay pane id from `start_overlay_pane`. Window bottom padding uses `has_visible_ai_chat_overlay()` (any AI chat on the active tab), not only the focused pane.
- Cmd+L, `#` (`inline_ai`), and `k` (`cli_chat`) share `AssistantConfig::auth_ready()`. Cmd+L and the `k` Engine share `ai_chat_engine::local_tools_allowed()` (remote OSC 7 cwd disables local tools); `#` is one-shot and has no tool surface. OSC 7 parsing lives in `ai_cwd.rs`. Inside Kaku, bare `k` opens Cmd+L (mux OSC 7). CLI `k` with args sets `Engine.remote_host` when `KAKU_OSC7_CWD` is exported.
