...
ensure_kaku_managed_yazi_indicator_padding() {
	if [[ ! -f "$YAZI_THEME_FILE" ]]; then
		return
	fi

	# Only patch themes generated by Kaku to avoid changing user-owned custom themes.
	if ! grep -q "^# Kaku-aligned theme for Yazi" "$YAZI_THEME_FILE"; then
		return
	fi

	local desired_padding='padding = { open = "", close = "" }'
	if grep -Fqx "$desired_padding" "$YAZI_THEME_FILE"; then
		return
	fi

	local tmp_theme_file
	tmp_theme_file="$(mktemp "${TMPDIR:-/tmp}/kaku-yazi-theme.XXXXXX")"

	awk -v desired="$desired_padding" '
		BEGIN { in_indicator = 0; saw_indicator = 0; replaced = 0 }

		/^[[:space:]]*\[indicator\][[:space:]]*$/ {
			if (in_indicator && !replaced) {
				print desired
				replaced = 1
			}
			in_indicator = 1
		...
		{
			if (in_indicator && $0 ~ /^[[:space:]]*padding[[:space:]]*=/) {
				print desired
				replaced = 1
				next
			}
		...
		END {
			if (!saw_indicator) {
				...
				print "[indicator]"
				print desired
			} else if (in_indicator && !replaced) {
				print desired
			}
		}
	' "$YAZI_THEME_FILE" >"$tmp_theme_file"

	mv "$tmp_theme_file" "$YAZI_THEME_FILE"
	echo -e "  ${GREEN}✓${NC} ${BOLD}Config${NC}      Updated Kaku Yazi indicator padding style"
}

ensure_kaku_managed_yazi_indicator_padding

ensure_kaku_managed_yazi_css_icon() {
	if [[ ! -f "$YAZI_THEME_FILE" ]]; then
		return
	fi
	...
	if grep -Eq 'name[[:space:]]*=[[:space:]]*"css"' "$YAZI_THEME_FILE"; then
		return
	fi

	# If user already defines [icon], do not merge automatically in shell script.
	if grep -Eq '^[[:space:]]*\[icon\][[:space:]]*$' "$YAZI_THEME_FILE"; then
		return
	fi
...
EOF
	...
	echo -e "  ${GREEN}✓${NC} ${BOLD}Config${NC}      Added CSS icon compatibility override for Yazi"
}
